...
### 2026-06-11 (续) (Claude Code) - 窄屏卡片改为撑满宽度、高度等比缩放
- **需求**(上一条的迭代):窄屏下卡片不再固定 540px 高居中,改为**宽度适应屏幕宽度、高度跟随宽度等比缩放**,背景仍与视频完全贴合。
- **改动**:`index.html`:
  - `<style>` 内 `.phone-mockup`(约 L129-150):基础样式改为 `width: 100%`,video 改 `width: 100%; height: auto;` 并加 `aspect-ratio: 720 / 1212`(两个视频实测都是 720×1212,声明固有比例避免视频元数据加载前高度跳动);`@media (min-width: 768px)` 里恢复 `width: auto; height: 90%`(video 同步恢复 `width: auto; height: 100%`)。
  - 两张卡片 div(现 L322、L339):去掉上一版的 `w-fit max-w-full mx-auto md:w-auto md:mx-0`,只留 `h-auto md:h-[600px] overflow-hidden`——窄屏卡片随文档流撑满内容区宽,高度由视频等比高度决定。
- **验证**(预览面板实测):375px 下卡片=视频=327×550.4;600px 下 552×929.2,宽高比都精确等于 720/1212≈0.5941;1280px 桌面端 596×600/视频 540 高居中,与改前一致。
- **注意**:在 600-767px 这种"宽但仍低于 md 断点"的窗口里卡片会跟着变得很高(等比放大的自然结果),属预期行为。
- **已上线**:提交 `a5e164c`,`~/deploy-backtap.sh` 推送+触发服务器更新(14:58),curl 线上确认 `aspect-ratio` 已在、旧版类名已消失。`.claude/launch.json` 仍未入库(用户尚未决定)。
